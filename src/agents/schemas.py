...
class PersonInfo(BaseModel):
    name: str = Field(
        description=(
            "Имя персоны в именительном падеже. Только на русском языке. "
            "ВАЖНО: сначала проверь список 'Известные персоны'. "
            "Если персона там есть — используй имя ДОСЛОВНО как в списке, "
            "даже если в тексте оно написано иначе (сокращено, в скобках, с титулом). "
            "Примеры совпадений: 'Григорий Трофимов' = 'Григорий (Трофимов)' = 'иеродиакон Григорий Трофимов'. "
            "Создавай новое имя только если персоны точно нет в списке известных."
        )
    )
    relation: str | None = Field(
        description=(
            "Роль персоны по отношению к автору. Одно слово или короткая фраза. "
            "Если персона есть в списке 'Известные персоны' — используй точно ту же роль. "
            "Если персона новая — опиши по аналогии с существующими: 'коллега', 'друг', 'врач', 'сосед', 'член семьи' и т.д. "
            "Если роль неизвестна — None."
        )
    )
    context: str | None = Field(
        description=(
            "1-2 предложения о конкретном упоминании персоны. "
            "Если персона упоминается вскользь — 'упомянут в контексте [краткая тема]'. "
            "ЗАПРЕЩЕНО копировать список известных персон или упоминать других людей."
            "Только факты. Никаких служебных пометок."
        )
    )
