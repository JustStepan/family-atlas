...
class PersonInfo(BaseModel):
    name: str = Field(
        description=(
            "Имя живого человека в именительном падеже, на русском. "
            "Убери титулы, звания, саны и обращения перед именем — оставь только имя "
            "(и фамилию, если она названа). Не сокращай и не дополняй."
        )
    )
    relation: str | None = Field(
        description=(
            "Роль человека одним словом или короткой фразой: 'сын', 'жена', 'коллега', "
            "'продавец', 'строитель'. Указывай ТОЛЬКО если роль ясна из текста. "
            "Если упомянут вскользь и роль не очевидна — None (лучше None, чем выдуманная)."
        )
    )
    context: str | None = Field(
        description="1-2 предложения только о фактах, сказанных про человека в этой заметке."
    )
...
class SessionBaseOutput(BaseModel):
    ...
    people_mentioned: list[PersonInfo] | None = Field(
            description=(
                "Реальные живые люди из текста. НЕ включай: автора заметки, AI-агентов, ботов, "
                "программы, сервисы и любые не-человеческие сущности (даже если у них есть имя). "
